...
interface BaseTransactionBatchItemProps {
...
function BaseTransactionBatchItem({
  testID,
  stock,
  activityName,
  isSelected,
  ...
  disableCollapse,
  onToggleDetail,
}: Readonly<BaseTransactionBatchItemProps>) {
  const { t } = useLanguage()

  const { batch, updated_at, available_qty, allocated_qty, qty } = stock
  const iconStyle = useAnimatedStyle(() => ({
    transform: [{ rotate: withTiming(isSelected ? '180deg' : '0deg') }],
  }))

  return (
    <View className='mx-4 border-quillGrey border p-3 mb-2 rounded-sm'>
      <TouchableOpacity
        activeOpacity={1}
      ...
      <CollapsableContainer expanded={disableCollapse ? true : isSelected}>
        <InfoRow
        ...
        {showAllocated && (
          <React.Fragment>
            <InfoRow
              ...
              value={numberFormat(allocated_qty)}
              valueClassName='uppercase'
            />
            <InfoRow
...
export default React.memo(BaseTransactionBatchItem)
